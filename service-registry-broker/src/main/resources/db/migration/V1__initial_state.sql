CREATE TABLE `credentials` (
  `id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `password` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `uri` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `username` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

CREATE TABLE `services` (
  `id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `bindable` bit(1) NOT NULL,
  `description` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `display_name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
  `image_url` longtext COLLATE utf8_unicode_ci,
  `long_description` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
  `provider_display_name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
  `name` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

CREATE TABLE `plans` (
  `id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `description` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `free` bit(1) NOT NULL,
  `name` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `service_id` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_med4ruw2v39cdl5flp25nglr3` (`service_id`),
  CONSTRAINT `FK_med4ruw2v39cdl5flp25nglr3` FOREIGN KEY (`service_id`) REFERENCES `services` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

CREATE TABLE `plan_metadata_bullets` (
  `plan_id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `bullet` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
  KEY `FK_1k0u7w45se2p4v4o6y6t6ev5x` (`plan_id`),
  CONSTRAINT `FK_1k0u7w45se2p4v4o6y6t6ev5x` FOREIGN KEY (`plan_id`) REFERENCES `plans` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

CREATE TABLE `plan_metadata_costs` (
  `plan_id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `usd` decimal(19,2) DEFAULT NULL,
  `unit` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
  KEY `FK_4j745fvwcs33fxsb1g15q6sf2` (`plan_id`),
  CONSTRAINT `FK_4j745fvwcs33fxsb1g15q6sf2` FOREIGN KEY (`plan_id`) REFERENCES `plans` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

CREATE TABLE `registered_services` (
  `id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `basic_auth_password` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `basic_auth_user` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `description` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `display_name` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `long_description` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `name` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `provider` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `url` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

CREATE TABLE `registered_service_features` (
  `registered_service_id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `feature` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
  KEY `FK_t8daqmw23q5gwwyrqf5wpf3rm` (`registered_service_id`),
  CONSTRAINT `FK_t8daqmw23q5gwwyrqf5wpf3rm` FOREIGN KEY (`registered_service_id`) REFERENCES `registered_services` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

CREATE TABLE `service_bindings` (
  `id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `app_guid` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `instance_id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `plan_id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `service_id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `service_binding_id` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_mh5u6eqd0ipu9ax2uw2wk5ncu` (`service_binding_id`),
  CONSTRAINT `FK_mh5u6eqd0ipu9ax2uw2wk5ncu` FOREIGN KEY (`service_binding_id`) REFERENCES `credentials` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

CREATE TABLE `service_instances` (
  `id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `organization_guid` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `plan_id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `service_id` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  `space_guid` varchar(255) COLLATE utf8_unicode_ci NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

insert into services (id, name, description, bindable, display_name, long_description, provider_display_name, image_url) values ('BD7097C1-F12A-11E3-AC10-0800200C9A66', 'service-registry', 'Service Registry for Applications', true, 'Service Registry', 'A service registry that will add your applications to the service marketplace.','Pivotal Software','data:image/png;base64,');
insert into plans (id, name, description, service_id, free) values ('BD7097C0-F12A-11E3-AC10-0800200C9A66', 'Standard', 'Standard Plan','BD7097C1-F12A-11E3-AC10-0800200C9A66', true);
insert into plan_metadata_bullets (plan_id, bullet) values ('BD7097C0-F12A-11E3-AC10-0800200C9A66', 'Binding to an instance of the registry service provides access to Registry API.');
insert into plan_metadata_bullets (plan_id, bullet) values ('BD7097C0-F12A-11E3-AC10-0800200C9A66', 'Registered applications will appear in marketplace.');
insert into plan_metadata_costs (plan_id, unit, usd) values ('BD7097C0-F12A-11E3-AC10-0800200C9A66', 'MONTH', 0.0);
